...
The commit message is a Markdown file with two parts: a summary line and an
optional body separated by a blank line.

### Summary Line

- Imperative mood ("Add search endpoint", not "Added search endpoint" or
  "Adds search endpoint")
- No conventional-commit prefixes — no `fix(scope):`, `feat:`, `chore:`, or
  any variant
- No trailing full stop
- ≤ 72 characters
- If the commit addresses a GitHub issue, append the issue number in
  parentheses at the end: `Enforce rate limiting on public API (#42)`

### Body

- Wrap at 72 characters
- Write in Markdown — use backticks for identifiers, fenced blocks for code,
  and lists where they aid clarity
- Imperative mood throughout ("Remove the fallback", not
  "Removed the fallback")
- Explain **what** changed and **why**, not how (the diff shows how)
- Reference related issues or pull requests (PRs) where relevant

### Attribution

Do not append any attribution line. No "Co-authored-by", "Signed-off-by",
"Generated by", or similar trailer attributing the message to an AI, a bot, or
any non-human agent. The commit message describes the change; it does not
advertise the toolchain.

## Hard Rules

These are non-negotiable. Violating any of them means the message is wrong.

- Never use `git commit -m` or `git commit -m "..."`.
  Multi-line messages are unreadable as shell arguments. The file-based
  workflow avoids quoting issues, supports proper formatting, and encourages
  thoughtful messages.
- Always write to a `mktemp -d` directory.
  This avoids polluting the working tree or repo root with scratch files.
- Use imperative mood in summary and body.
  This matches the convention established by Git itself (`Merge branch ...`,
  `Revert "..."`).
...
- Write the message file as Markdown.
  Markdown enables backtick formatting for identifiers and fenced code blocks
  when referencing specific symbols or snippets.
...
Writing to a file in a `mktemp -d` directory is the boring option. It is also
the correct one. The message is visible on disk before the commit, trivially
reviewable, immune to shell expansion, and cleaned up afterwards. Better to be
correct than to be clever.
...
3. **What alternatives were considered or rejected?** Include if it saves a
   future reader from retreading the same ground.
4. **Are there non-obvious consequences?** Migration steps, deprecations, perf
   implications — mention them.

For trivial changes (typo fixes, import reordering), a summary line alone
suffices. Do not pad with filler.
...
```text
Replace hand-rolled least recently used (LRU) cache with `lru-cache` crate
